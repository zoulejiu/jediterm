...
public class TerminalTypeAheadManagerTest extends TestCase {
  ...
  public void testPasswordPromptDetection() throws Exception {
    new TestRunner() {
      @Override
      void run() {
        manager.onKeyEvent(TerminalTypeAheadManager.TypeAheadEvent.fromChar('a'));
        assertFalse(didDrawPredictions());
        model.insertString("a");
        manager.onTerminalStateChanged();
        manager.onKeyEvent(TerminalTypeAheadManager.TypeAheadEvent.fromChar('b'));
  ...
  public void testLowLatency() {
    new TestRunner() {
      ...
      void run() {
        ...
        TerminalTypeAheadManager.TypeAheadEvent.fromString("type ahead").forEach(manager::onKeyEvent);
  ...
  public void testBackspacePrediction() throws Exception {
    new TestRunner() {
      ...
      void run() {
        ...
        manager.onKeyEvent(new TerminalTypeAheadManager.TypeAheadEvent(TerminalTypeAheadManager.TypeAheadEvent.EventType.Backspace));
  ...
  public void testCursorMovePrediction() throws Exception {
    new TestRunner() {
      ...
      void run() {
        model.currentLine += 'a';
        manager.onKeyEvent(new TerminalTypeAheadManager.TypeAheadEvent(TerminalTypeAheadManager.TypeAheadEvent.EventType.RightArrow));
  ...
  public void testTentativeCursorMovePrediction() throws Exception {
    new TestRunner() {
      ...
      void run() {
        model.insertString("a");
        manager.onKeyEvent(new TerminalTypeAheadManager.TypeAheadEvent(TerminalTypeAheadManager.TypeAheadEvent.EventType.LeftArrow));
  ...
  abstract static class TestRunner {
    ...
    TestRunner fillLatencyStats() throws NoSuchFieldException, IllegalAccessException {
      ...
      TerminalTypeAheadManager.LatencyStatistics myLatencyStatistics = (TerminalTypeAheadManager.LatencyStatistics) myLatencyStatisticsField.get(manager);

      Field myLatenciesField = TerminalTypeAheadManager.LatencyStatistics.class.getDeclaredField("myLatencies");
